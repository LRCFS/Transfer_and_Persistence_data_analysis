...
## Plotting of Transfer Experiment Results - for A

Starting with the simplest dataset e.g. with few samples and combinations. Using the combined spreadsheet "Counts_data.xlsx as all the data should be in there and is the source of the paper's figures.
...
knitr::kable(head(transfer.dat, 20), caption = "'Head' of A Transfer Data")
...
In trying to replicate the above for C data, I found that the "Time" column didn't make sense (an incrementing value per row) so from now on will use the cleaned data which was used for the file-renamer.
...
So a simulation (see the 'simul.Rmd' doc) suggests that when P3 - P1 approaches 0 the Transfer Ratio values go very high. Let's see what the distribution of P3 - P1 looks like in all our experiments.
...
There are no standout differences between the experiment. Oh well...
...
ggsave('figure2.tiff', trp12,  height = 4.5, units = 'in', dpi=300)
```
...
ggsave("figure3.tiff", height = 4.5, units = 'in', dpi=300)
...
tep12 = ggarrange(tep1, tep2, labels = c('A', 'B'), ncol = 2,
                  common.legend = TRUE)
tep12
ggsave("figure4.tiff", tep12, height = 4.5, units = 'in', dpi=300)
