...
all.dat = cbind(Name = rep("A",nrow(merge.dat)), merge.dat)
...
all.ratio = cbind(Name = rep("A", 2), Mass = rep(1000,2), TransferTime = rep(60,2), Substrate = c("Wool01", "Nylo01"), all.ratio)

# do same for efficiency data
all.eff = eff.dat
all.eff = all.eff[, -1]
all.eff = cbind(Name = rep("A", 2), Mass = rep(1000,2), TransferTime = rep(60,2), Substrate = c("Wool01", "Nylo01"), all.eff)
...
all.dat = rbind(all.dat, data.frame(Name = rep("B",nrow(merge.dat)), merge.dat))
...
ratio.dat = cbind(Name = rep("B", nrow(ratio.dat)), ratio.dat)
...
eff.dat = cbind(Name = rep("B", nrow(eff.dat)), eff.dat)
all.eff = rbind(all.eff, eff.dat)
...
all.dat = rbind(all.dat, data.frame(Name = rep("C",nrow(merge.dat)), merge.dat))
...
ratio.dat = summarySE(final.dat, measurevar = "Ratio", groupvars = c("Mass","TransferTime","Substrate"))
ratio.dat = cbind(Name = rep("C", nrow(ratio.dat)), ratio.dat)
all.ratio = rbind(all.ratio, ratio.dat)
...
eff.dat = summarySE(final.dat, measurevar = "Efficiency", groupvars = c("Mass","TransferTime","Substrate"))
eff.dat = cbind(Name = rep("C", nrow(eff.dat)), eff.dat)
all.eff = rbind(all.eff, eff.dat)
knitr::kable(eff.dat, caption = "Efficiency data for SD")
...
all.dat = rbind(all.dat, data.frame(Name = rep("D",nrow(merge.dat)), merge.dat))
...
ratio.dat = summarySE(final.dat, measurevar = "Ratio", groupvars = c("Mass","TransferTime","Substrate"))
ratio.dat = cbind(Name = rep("D", nrow(ratio.dat)), ratio.dat)
all.ratio = rbind(all.ratio, ratio.dat)
...
eff.dat = summarySE(final.dat, measurevar = "Efficiency", groupvars = c("Mass","TransferTime","Substrate"))
eff.dat = cbind(Name = rep("D", nrow(eff.dat)), eff.dat)
all.eff = rbind(all.eff, eff.dat)
...
all.dat = rbind(all.dat, data.frame(Name = rep("E",nrow(merge.dat)), merge.dat))
...
ratio.dat = summarySE(final.dat, measurevar = "Ratio", groupvars = c("Mass","TransferTime","Substrate"))
ratio.dat = cbind(Name = rep("E", nrow(ratio.dat)), ratio.dat)
all.ratio = rbind(all.ratio, ratio.dat)
...
eff.dat = summarySE(final.dat, measurevar = "Efficiency", groupvars = c("Mass","TransferTime","Substrate"))
eff.dat = cbind(Name = rep("E", nrow(eff.dat)), eff.dat)
all.eff = rbind(all.eff, eff.dat)
knitr::kable(eff.dat, caption = "Efficiency data for SD")
...
all.ratio.tmp = all.dat %>% group_by(Name, Mass, TransferTime, Experiment, Substrate, Replicate) %>%
  summarise(Donor = P3 - P1, Receiver = P5 - P2, Donor_post = P3 - P4) %>% mutate(Ratio = Receiver/Donor, Efficiency = Receiver/Donor_post)

# aggregate over replicates
all.final = all.ratio.tmp %>% 
  group_by(Name, Mass, TransferTime, Substrate) %>%
  summarise(N = n(), ratio = mean(Ratio), se = stdErr(Ratio))

wool.dat = all.final %>% filter(Mass == 1000 & Substrate == "Wool01")
wool.dat
...
nylon.dat = all.final %>% filter(Mass == 1000 & Substrate == "Nylo01")
nylon.dat
...
wool60.dat = all.final %>% filter(TransferTime == 60 & Substrate == "Wool01")
wool60.dat
...
nylon60.dat = all.final %>% filter(TransferTime == 60 & Substrate == "Nylo01")
nylon60.dat
...
# plot boxplots
ggplot(all.ratio.tmp %>% 
  filter(Substrate != "Deni01"), aes(x=Name, y=Ratio, fill = Substrate)) +
  geom_boxplot() +
  labs(title = "Transfer Ratio by Experiment and Substrate",
       x = "Experiment",
       y = "Transfer Ratio") +
  mytheme
...
```{r plotallratio, fig.width=3.5}

# for better labelling add a new mass column with a 'g'
all.final$Massg = paste0(all.final$Mass, "g")

ggplot(all.final, aes(x = numericFactorOrder(TransferTime), y = ratio, group = Name, colour = Name)) + 
...
nylon.eff.dat = all.eff %>% filter(Mass == 1000 & Substrate == "Nylo01")
nylon.eff.dat

ggplot(nylon.eff.dat, aes(x=numericFactorOrder(TransferTime), y = Efficiency, group = Name, colour = Name, shape=Name)) +
  geom_point(size = 2, position = position_dodge(0.3)) +
...
ggplot(all.eff, aes(x = numericFactorOrder(TransferTime), y = Efficiency, group = Name, colour = Name)) + 
  geom_point(size=1, position = position_dodge(0.3)) +
  geom_errorbar(aes(ymin = Efficiency-se, ymax = Efficiency+se), width = 0.08, position = position_dodge(0.3)) +
#  geom_point(colour = "white", size = 0.7, position = position_dodge(0.2)) +
#  scale_x_discrete(labels = c("Wool", "Nylon")) +
