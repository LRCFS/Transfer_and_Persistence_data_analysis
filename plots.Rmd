...
final.dat = merge.dat %>% group_by(Mass, Duration, Group, Repeat) %>% summarise(Mass, Duration, Group, Repeat, Donor = P3 - P1, Receiver = P5 - P2, Donor_post = P3 - P4) %>% mutate(Ratio = Receiver/Donor, Efficiency = Receiver/Donor_post)
...
```{r readBh}
# Read in ML's data
my.dat = read_xlsx("dat/20191030DatasetPaper.xlsx", sheet = "DataBh", trim_ws = TRUE, col_types = "text")

# FIXME - this isn't working
# Fix the column name for Time - seems to have extra spaces
rename(my.dat, Time = Time...10)
colnames(my.dat)

# only use the Transfer data (Time == "0000") 
transfer.dat = my.dat %>% filter(Time == "00000") %>% select(c("Substrate", "ActivityID", "ObsType", "Count", "Mass", "Duration", "Group", "Repeat")) 
transfer.dat
